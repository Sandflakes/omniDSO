...
int main(int argc, char **argv)
{
    ...
    std::thread runthread([&]()
        ...
        while(true)
        {
            ...
            {
                if (frameCounter < 250 || setting_fullResetRequested)
                {
                    printf("RESETTING!\n");
        ...
        if (setting_logStuff)
        {
            ...
            tmlog << 1000.0f * (ended - started) / (float)(CLOCKS_PER_SEC * frameCounter) << " "
                  << ((tv_end.tv_sec - tv_start.tv_sec) * 1000.0f + (tv_end.tv_usec - tv_start.tv_usec) / 1000.0f) / (float)frameCounter << "\n";
            tmlog.flush();
            tmlog.close();
        } });
